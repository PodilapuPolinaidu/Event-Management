...
const FeaturedEvents = ({ alternate = false }) => {
  return (
    <Section alternate={alternate}>
      <SectionHeader
        title="Featured Event Packages"
        subtitle="Discover our most popular event packages designed to make your occasion extraordinary"
        ...
        {events.map((event, index) => (
          <Card key={index} className="overflow-hidden hover:shadow-elegant transition-all duration-300 group">
            ...
            <CardFooter className="p-4 pt-0">
              <span className="w-full text-primary hover:text-primary-dark font-semibold text-center cursor-pointer flex items-center justify-center gap-2 group/link transition-colors">
                View Details
